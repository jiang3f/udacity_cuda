...
__global__ void foo(float out[], float A[], float B[], float C[], float D[], float E[])
{
	...
	__shared__ float tileC[BLOCKSIZE];
	__shared__ float tileD[BLOCKSIZE];
	__shared__ float tileE[BLOCKSIZE];

	int i = threadIdx.x + blockIdx.x*blockDim.x;
	tileA[threadIdx.x] = A[i];
	tileB[threadIdx.x] = B[i];
	tileC[threadIdx.x] = C[i];
	tileD[threadIdx.x] = D[i];
	tileE[threadIdx.x] = E[i];

	__syncthreads();

	out[i] = (tileA[threadIdx.x] + tileB[threadIdx.x] + tileC[threadIdx.x] + tileD[threadIdx.x] + tileE[threadIdx.x]) / 5.0f;
}

__global__ void bar(float out[], float in[])
{
#if 1
	__shared__ float tile[BLOCKSIZE+4];

	int myId = (blockIdx.x * blockDim.x) + threadIdx.x;

	tile[threadIdx.x+2] = in[myId];
	...
	{
		tile[threadIdx.x + 3] = in[myId + 1];
		tile[threadIdx.x + 4] = in[myId + 2];
	}

	__syncthreads();

	out[myId] = (tile[threadIdx.x] + tile[threadIdx.x + 1] + tile[threadIdx.x+2] + tile[threadIdx.x + 3] + tile[threadIdx.x + 4]) / 5.0f;
#else
	int myId = (blockIdx.x * blockDim.x) + threadIdx.x;

	out[myId] = (in[myId - 2] + in[myId - 1] + in[myId] + in[myId + 1] + in[myId + 2]) / 5.0f;
#endif
}
